...
uint8_t mbuf_protocol(struct rte_mbuf *mbuf)
{
    ...
    if (etype == ETHER_TYPE_IPv4)
        return ((struct rte_ipv4_hdr *)l3_hdr)->next_proto_id;
    if (etype == ETHER_TYPE_IPv6)
        // TODO: support ipv6 extension headers
        return ((struct rte_ipv6_hdr *)l3_hdr)->proto;
    return 0;
}

void *mbuf_header_l2(struct rte_mbuf *mbuf)
{
    struct rt_ether_hdr *ehdr;
    ...
    switch (ethtype) {
        case ETHER_TYPE_IPv4:
        {
            ...
            if (unlikely(mbuf->data_len < mbuf->l2_len + sizeof(*ip4hdr)))
                return NULL;
            ip4hdr = rte_pktmbuf_mtod_offset(mbuf, struct rte_ipv4_hdr *, mbuf->l2_len);
        ...
        case ETHER_TYPE_IPv6:
        {
            ...
            offset = mbuf->l2_len + sizeof(struct rte_ipv6_hdr);
        ...
        case ETHER_TYPE_ARP:
        {
            int l3_len = sizeof(struct rte_arp_hdr);
            if (unlikely(mbuf->data_len < mbuf->l2_len + l3_len))
                return NULL;
            mbuf->l3_len = l3_len;
