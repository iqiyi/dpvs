...
void dp_vs_conn_dump(const char *msg, struct dp_vs_conn *conn)
{
    ...
    caddr = inet_ntop(conn->tuplehash[DPVS_CONN_DIR_INBOUND].af,
                      &conn->caddr, cbuf, sizeof(cbuf)) ? cbuf : "::";
    vaddr = inet_ntop(conn->tuplehash[DPVS_CONN_DIR_INBOUND].af,
                      &conn->vaddr, vbuf, sizeof(vbuf)) ? vbuf : "::";
    laddr = inet_ntop(conn->tuplehash[DPVS_CONN_DIR_OUTBOUND].af,
                      &conn->laddr, lbuf, sizeof(lbuf)) ? lbuf : "::";
    daddr = inet_ntop(conn->tuplehash[DPVS_CONN_DIR_OUTBOUND].af,
                      &conn->daddr, dbuf, sizeof(dbuf)) ? dbuf : "::";
