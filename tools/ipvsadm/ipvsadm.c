...
static int parse_acl_rule(const char *buf, ipvs_service_t *svc)
{
	...
	for (start = params; (arg = strtok_r(start, ",", &sp)); start = NULL) {
		...
		if (!strcmp(key, "set")) {
			...
			svc->user.black = !strcmp(val, "black")? true : false;	
...
static void usage_exit(const char *program, const int exit_status)
{
	...
	fprintf(stream,
		...
		"  --add-acl         -K        add IPset based ACL rule\n"
	...
	fprintf(stream,
		...
		"  --acl-entry    -3                   ACL entry is 'set,type,prio,dir'\n",
		DEF_SCHED);
...
static int list_aclrules(unsigned int format, ipvs_service_t *svc)
{
	...
	for (i = 0; i < get->num_rules; i++) {
		...
		printf("  %d\t%-32s%s\t%s\n", 
			...
			get->entrytable[i].black? "blacklist" : "whitelist", dir);
	}
