...
static void
acl_group_handler(const vector_t *strvec)
{
	if (!strvec)
		return;
	alloc_acl_group(vector_slot(strvec, 1));
	alloc_value_block(alloc_acl_entry, strvec_slot(strvec, 0));
}

static void
acl_gname_handler(const vector_t *strvec)
{
	virtual_server_t *vs = LIST_TAIL_DATA(check_data->vs);
	vs->acl_rule_gname = set_value(strvec);
}
...
init_check_keywords(bool active)
{
	...
	/* ACL */
	install_keyword_root("acl_rule_group", &acl_group_handler, active);
	...
	install_keyword("acl_group_name", &acl_gname_handler);
