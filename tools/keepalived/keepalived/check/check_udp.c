...
static bool
compare_udp_check(const checker_t *a, const checker_t *b)
{
...
udp_check_handler(__attribute__((unused)) const vector_t *strvec)
{
	...
	queue_checker(free_udp_check, dump_udp_check, udp_connect_thread,
	            compare_udp_check, udp_check, CHECKER_NEW_CO(), true);
}
...
payload_handler(const vector_t *strvec)
{
	...
	udp_check->payload = STRDUP((const char*)hex_str);
	udp_check->payload_len = strlen(hex_str); //read_hex_str(hex_str, &udp_check->payload, NULL);
...
require_reply_handler(const vector_t *strvec)
{
	...
	hex_str = make_strvec_str(strvec, 1);
	udp_check->reply_data = STRDUP((const char*)hex_str);
	udp_check->reply_len = strlen(hex_str); //read_hex_str(hex_str, &udp_check->reply_data, &udp_check->reply_mask);
...
check_udp_reply(const uint8_t *recv_data, size_t len, const udp_check_t *udp_check)
{
	...
	for (i = 0; i < check_len; i++) {
		if ((recv_data[i] ^ udp_check->reply_data[i]))
			return true;
	}
...
udp_check_thread(thread_ref_t thread)
{
    ...
    len = udp_check->require_reply ? (udp_check->max_reply_len + 1) : 1;
	recv_buf = MALLOC(len);
