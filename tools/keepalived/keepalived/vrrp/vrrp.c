...
vrrp_handle_ipaddress(vrrp_t * vrrp, int cmd, int type)
{
	...
	netlink_iplist((type == VRRP_VIP_TYPE) ? vrrp->vip : vrrp->evip, cmd, vrrp->dpdk_ifp);
	return 1;
}
...
clear_diff_vrrp_vip(vrrp_t * old_vrrp, int type)
{
    ...
    strcpy (old_dpdk_port, old_vrrp->dpdk_ifp);
    strcpy (dpdk_port, vrrp->dpdk_ifp);

	clear_diff_address(l, n, old_dpdk_port, dpdk_port);
}
