...
def dpdk_env_setup(FILE_CONTENT, ifaces):
    ...
    for nic in niclist:
        ...
        elif nic.find("drv=ixgbe") == -1 or nic.find("drv=igb_uio") == -1:  #networks not other or kernel
...
def dpvs_ctrl(bind_num, FILE_CONTENT, DPVS_VERSION, gitpro = False, spec = ""):
    ...
    if bind_num >=2:
        cp_cmd = "cp -f ~/dpvs.conf /etc/dpvs.conf"
    else:
        return {"status":False, "content":FILE_CONTENT, "detail":"DPVS DPDK bind_num WRONG"}
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        print "usage:"
        print "  python dpvs-setup-ctrl.py DPVS_GIT_VERSION(checkout branch then make) ifaces bond0/1"
    if len(sys.argv) > 1:
