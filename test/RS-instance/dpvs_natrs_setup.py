...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 4:
        print "usage:"
        print "  python dpvs_natrs_setup.py ip1,...,ipn route wan_ip"
    ...
    file_name = ENV_FILE_DIR + "/rollback_natrs.sh"
    FILE_CONTENT = ""
    num = 0
    for ip in ip_list:
    ...
    route = sys.argv[2]
    wan_ip = sys.argv[3]
    cmd = "ip route add " + route + " via " + wan_ip + " dev " + iface
    if os.system(cmd) != 0:
        file_writen(FILE_CONTENT, file_name)
        print "route config failed!"
        sys.exit(-1)
    cmd0 = "ip route del " + route + " via " + wan_ip + " dev " + iface + "\n"
    FILE_CONTENT = cmd0 + FILE_CONTENT
